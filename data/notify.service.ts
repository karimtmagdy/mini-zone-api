...
export class NotifyService {
  ...
  async sendAccountLockedNotification(email: string, username: string) {
    await this.emailService
      ...
      .catch((err) =>
        logger.error("Failed to send account locked email:", err),
      );
  }

  async notifyAdminNewUser() {
    // Implement admin notification logic (e.g., email to admin or push)
    // const admins = await this.userService.getAll();
    //       const admins = await this.userService.getUsers();
    //   const adminEmails = admins.filter((u) => u.role === "admin");
    //   for (const admin of adminEmails) {
    //     if (admin.email) {
    //       await this.emailService.sendNotificationEmail(
    //         admin.email,
    //         "New User Registered",
    //         `New user registered: ${newUserEmail}`
    //       );
    //     }
    //   }
    // }
  }
  //   static async notifyAccountDeletion(
  //   email: string,
  //   deletedBy: string
  // ): Promise<void> {
  ...
  //     email,
  //     "Account Deleted",
  //     `Your account has been deleted by ${deletedBy}`
  //   );
  // }
}
