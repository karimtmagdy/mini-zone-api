...
export class ProductRepoImpl implements ProductRepoType {
  ...
  private toEntity(doc: IProduct): Product {
    return new Product({
      ...
      sold: doc.sold,
      colors: doc.colors,
      category: (this.mapEntityRef(doc.category) ?? doc.category) as any,
      subcategory: this.mapEntityRefs(doc.subcategory) as any,
      brand: (this.mapEntityRef(doc.brand) ?? doc.brand) as any,
      ratings: doc.ratings,
      reviews: doc.reviews as any,
      tags: doc.tags,
